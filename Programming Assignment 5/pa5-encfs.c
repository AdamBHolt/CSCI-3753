/* 
* CS3752 PA5 - Adam Holt & Joe Cosenza
* This is an exectuable that allows a user file system to be mounted mirroring an existing directory
* using an empty directory as a mount point and taking a passphrase as an argument.
* Files in the mounted file system are encrypted by default
...
/* Change paths of the functions to the specific mirror directory */
...
/* Get characteristics of a file and stores them in a struct */
...
{
	int res;
	/* change path to specific mirror directory */
	char fpath[PATH_MAX];
	xmp_fullpath(fpath, path);
...
int main(int argc, char *argv[])
{
	...
	if(argc < 4){
    	...
    	fprintf(stdout, "Mirror Directory = %s\n", xmp_data->mirror_dir);
